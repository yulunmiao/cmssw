...
HGCalDenseMappingTool::HGCalDenseMappingTool(HGCalDenseMappingToolConfig config)
    : config_(config){}
...
uint32_t HGCalDenseMappingTool::denseMap(uint32_t sLink, uint32_t captureBlock, uint32_t eCOND, uint32_t eRx, uint32_t channel){
  uint32_t rtn=sLink;
  rtn = rtn * config_.sLinkCaptureBlockMax + captureBlock;
  rtn = rtn * config_.captureBlockECONDMax + eCOND;
  rtn = rtn * config_.econdERXMax + eRx;
